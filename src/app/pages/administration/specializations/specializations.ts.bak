...
export class AdminSpecializationsComponent implements OnInit {
  ...
  selectedPermissions: Specialization[] = [];
  loading = false;
  error: string | null = null;
  displayDialog = false;
  editModel: Partial<Specialization> = {};
  isCreating = false;
  ...
  private safeDetect(): void { try { this.cd.detectChanges(); } catch (e) { console.warn('safeDetect failed', e); } }

  ngOnInit(): void { this.loadPermissions(); }

  onGlobalFilter(table: any, event: Event): void { const val = (event && (event.target as HTMLInputElement)) ? (event.target as HTMLInputElement).value : ''; try { table.filterGlobal(val, 'contains'); } catch (e) { console.warn('onGlobalFilter failed', e); } }

  loadPermissions(): void { this.loading = true; this.error = null; this.specializationsService.getPermissions().subscribe({ next: (data: any) => { this.permissions = (data && (data as any).data) ? (data as any).data : (data || []); this.loading = false; this.safeDetect(); }, error: (err: any) => { console.error('Failed to load specializations:', err); this.error = (err && err.message) ? err.message : 'Failed to load items'; this.loading = false; this.safeDetect(); } }); }
  ...
  confirmDelete(item: Specialization): void { if (!item) return; this.confirmationService.confirm({ message: `${this.translate.instant('components.permissions.confirm.DELETE_QUESTION') || 'Delete item'} "${item.name || item.id}"?`, icon: 'pi pi-exclamation-triangle', accept: () => this.deletePermission(item) }); }

  deletePermission(item: Specialization): void { if (!item) return; this.loading = true; this.specializationsService.deletePermission(item.id).subscribe({ next: (_res: any) => { this.permissions = this.permissions.filter(u => u.id !== item.id); this.selectedPermissions = this.selectedPermissions.filter(s => s.id !== item.id); this.loading = false; try { this.messageService.add({ severity: 'success', summary: this.translate.instant('MENU.DELETE') || 'Delete', detail: 'Deleted' }); } catch (e) {} this.safeDetect(); }, error: (err: any) => { console.error('Failed to delete item', err); this.error = (err && err.message) ? err.message : 'Failed to delete item'; this.loading = false; this.safeDetect(); } }); }
  ...
  exportCSV(): void {
    try {
      ...
      if (!rows.length) {
        try {
          this.messageService.add({
            severity: 'info',
            summary: this.translate.instant('MENU.EXPORT') || 'Export',
            detail: this.translate.instant('MENU.ANY') || 'No users to export'
          });
        } catch (e) {}
        return;
      }
      ...
      const headers = [
        ...
        this.translate.instant('components.permissions.table.HEADERS.CODE') || 'Code',
      ...
      for (const u of rows) {
        const line = [esc(u.id), esc(u.name), esc((u as any).code), esc((u as any).description), esc(u.created_at)].join(',');
        lines.push(line);
      }
      ...
      const filename = `specializations_export_${timestamp}.csv`;
      ...
      try {
        this.messageService.add({
          severity: 'success',
          summary: this.translate.instant('MENU.EXPORT') || 'Export',
          detail: this.translate.instant('components.permissions.messages.EXPORTED') || 'Export completed'
        });
      } catch (e) {}
    } catch (err) {
      console.error('Export failed', err);
      try {
        this.messageService.add({ severity: 'error', summary: this.translate.instant('MENU.EXPORT') || 'Export', detail: 'Export failed' });
      } catch (e) {}
    }
  }
  ...
  validateForm(): boolean {
    ...
    const code = (this.editModel && (this.editModel as any).code) ? String((this.editModel as any).code).trim() : '';

    if (!name) {
      this.formErrors.name = (this.translate.instant('components.permissions.form.NAME') || 'Name') + ' is required';
    }
    if (!code) {
      this.formErrors.code = (this.translate.instant('components.permissions.form.CODE') || 'Code') + ' is required';
    }
