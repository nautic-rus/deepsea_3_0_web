...
export class AdminDepartmentsComponent implements OnInit {
  ...
  savePermission(): void {
    ...
    if (!this.validateForm()) { try { this.messageService.add({ severity: 'error', summary: this.translate.instant('MENU.CONFIRM') || 'Error', detail: 'Please fix form errors' }); } catch (e) {} return; }
    this.loading = true;
    if (this.isCreating) {
      this.departmentsService.createPermission(payload as any).subscribe({
        next: (_res: any) => { this.displayDialog = false; this.editModel = {} as any; this.loading = false; this.isCreating = false; try { this.messageService.add({ severity: 'success', summary: this.translate.instant('MENU.CREATE') || 'Success', detail: 'Created' }); } catch (e) {} this.loadPermissions(); this.safeDetect(); },
        error: (err: any) => { console.error('Failed to create item', err); this.error = (err && err.message) ? err.message : 'Failed to create item'; this.loading = false; try { this.messageService.add({ severity: 'error', summary: this.translate.instant('MENU.CREATE') || 'Create', detail: (err && err.message) ? err.message : 'Failed to create permission' }); } catch (e) {} this.safeDetect(); }
      });
    } else {
      this.departmentsService.updatePermission(id as any, payload as any).subscribe({
        next: (_res: any) => { this.displayDialog = false; this.editModel = {} as any; this.loading = false; try { this.messageService.add({ severity: 'success', summary: this.translate.instant('MENU.SAVE') || 'Success', detail: 'Updated' }); } catch (e) {} this.loadPermissions(); this.safeDetect(); },
        error: (err: any) => { console.error('Failed to update item', err); this.error = (err && err.message) ? err.message : 'Failed to update item'; this.loading = false; try { this.messageService.add({ severity: 'error', summary: this.translate.instant('MENU.SAVE') || 'Save', detail: (err && err.message) ? err.message : 'Failed to update permission' }); } catch (e) {} this.safeDetect(); }
      });
    }
  }

  confirmDelete(item: Department): void {
    if (!item) return; this.confirmationService.confirm({ message: `${this.translate.instant('components.permissions.confirm.DELETE_QUESTION') || 'Delete item'} "${item.name || item.id}"?`, icon: 'pi pi-exclamation-triangle', accept: () => this.deletePermission(item) });
  }
  ...
  exportCSV(): void {
    try { const rows = this.permissions || []; if (!rows.length) { try { this.messageService.add({ severity: 'info', summary: this.translate.instant('MENU.EXPORT') || 'Export', detail: this.translate.instant('MENU.ANY') || 'No users to export' }); } catch (e) {} return; }
