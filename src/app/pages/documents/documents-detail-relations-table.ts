...
export class DocumentsDetailRelationsTableComponent implements OnChanges {
  ...
  private loadLinksForDocument(doc: any | null) {
    ...
    const reqSource = this.linksService.getLinks({ active_type: 'document', active_id: String(id) }).pipe(catchError(() => of([])));
    const reqTarget = this.linksService.getLinks({ passive_type: 'document', passive_id: String(id) }).pipe(catchError(() => of([])));
  ...
  public removeLink(rel: any): void {
    ...
    let msg = '';
    try {
      msg = this.translate.instant('components.documents.relations.DELETE_CONFIRM_WITH_NAME', { type, id, name });
      if (!msg || msg === 'components.documents.relations.DELETE_CONFIRM_WITH_NAME') {
        const base = this.translate.instant('components.documents.relations.DELETE_CONFIRM') || 'Delete this relation?';
        msg = `${base} ${type ? type + ' ' : ''}${id ? ('#' + id) : ''}${name ? ' - ' + name : ''}`.trim();
      }
    } catch (e) {
      msg = `Delete ${type ? type + ' ' : ''}${id ? ('#' + id) : ''}${name ? ' - ' + name : ''}?`;
    }
    ...
    try {
      ...
      // fallback to native confirm if ConfirmationService fails
