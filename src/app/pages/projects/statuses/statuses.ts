...
export class ProjectsStatusesComponent implements OnInit, AfterViewInit {
  projects: any[] = [];
  ...
  ngOnInit(): void {
    try {
      this.auth.me().subscribe({ next: (res: any) => {
  ...
  private loadUsersForSelect(): void {
    try {
      this.usersService.getUsers(1, 1000).subscribe({ next: (res: any) => {
  ...
  private loadOwnerNames(): void {
    try {
      ...
      forkJoin(calls).subscribe({ next: (responses: any[]) => {
  ...
  confirmDelete(item: any): void {
    if (!item) return;
    try {
      this.confirmationService.confirm({ message: `${this.translate.instant('components.projects.confirm.DELETE_QUESTION') || 'Attention! Do you really want to delete project'} ${item.name || item.id}?`, icon: 'pi pi-exclamation-triangle', accept: () => this.deleteItem(item) });
    } catch (e) { this.deleteItem(item); }
  }
