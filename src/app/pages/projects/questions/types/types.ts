...
export class QuestionTypesComponent implements OnInit {
  ...
  constructor(private svc: CustomerQuestionTypesService, private cd: ChangeDetectorRef, private appMsg: AppMessageService, private translate: TranslateService, private confirm: ConfirmationService) {}
  ...
  save(): void {
    ...
    if (this.isCreating) {
      this.svc.createType(payload).subscribe({ next: () => { this.displayDialog = false; this.load(); this.appMsg.success('Created'); this.loading = false; }, error: (e: any) => { this.error = e?.message || 'Error'; this.loading = false; } });
    } else {
      const id = this.editModel.id || this.editModel._id;
      this.svc.updateType(id, payload).subscribe({ next: () => { this.displayDialog = false; this.load(); this.appMsg.success('Updated'); this.loading = false; }, error: (e: any) => { this.error = e?.message || 'Error'; this.loading = false; } });
    }
  }
  ...
  confirmDelete(item: any): void {
    this.confirm.confirm({ message: `${this.translate.instant('components.projects.settings.confirm.DELETE_QUESTION') || 'Attention! Do you really want to delete'} ${item.name || item.id}?`, icon: 'pi pi-exclamation-triangle', accept: () => {
      this.svc.deleteType(item.id).subscribe({ next: () => { this.load(); this.appMsg.success('Deleted'); }, error: () => {} });
    } });
  }
