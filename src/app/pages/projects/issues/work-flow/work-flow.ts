...
export class IssueWorkFlowComponent implements OnInit {
  items: any[] = [];
  loading = false;
  displayDialog = false;
  editModel: any = {};
  ...
  private loadStatuses(): void {
    this.statusesSvc.getStatuses().subscribe({ next: (res: any) => {
  ...
  private loadTypes(): void {
    this.typesSvc.getIssueTypes().subscribe({ next: (res: any) => {
      const arr = (res && res.data) ? res.data : (Array.isArray(res) ? res : []);
  ...
  private load(): void {
    this.loading = true;
    this.svc.getIssueWorkFlows().subscribe({ next: (res: any) => {
  ...
  private buildFilterOptions(): void {
    ...
    this.items.forEach((it: any) => {
      ...
      const t = it?.issue_type?.name || it?.issue_type?.code || (it?.issue_type_id != null ? String(it.issue_type_id) : null);
  ...
  openNew(): void { this.editModel = { project_id: null, issue_type_id: null, from_status_id: null, to_status_id: null }; this.isCreating = true; this.displayDialog = true; this.error = null; }
  openEdit(item: any): void { this.editModel = { ...item, project_id: item?.project?.id || item?.project_id, issue_type_id: item?.issue_type?.id || item?.issue_type_id }; this.isCreating = false; this.displayDialog = true; this.error = null; }

  save(): void {
    this.loading = true;
    const payload: any = { project_id: this.editModel.project_id, issue_type_id: this.editModel.issue_type_id, from_status_id: this.editModel.from_status_id, to_status_id: this.editModel.to_status_id };
    if (this.isCreating) {
      this.svc.createIssueWorkFlow(payload).subscribe({ next: () => { this.displayDialog = false; this.load(); this.msg.add({ severity: 'success', summary: 'Created' }); this.loading = false; }, error: (e: any) => { this.error = e?.message || 'Error'; this.loading = false; } });
    } else {
      const id = this.editModel.id || this.editModel._id;
      this.svc.updateIssueWorkFlow(id, payload).subscribe({ next: () => { this.displayDialog = false; this.load(); this.msg.add({ severity: 'success', summary: 'Updated' }); this.loading = false; }, error: (e: any) => { this.error = e?.message || 'Error'; this.loading = false; } });
    }
  }

  confirmDelete(item: any): void {
    this.confirm.confirm({ message: `Delete workflow #${item.id}?`, accept: () => {
      this.svc.deleteIssueWorkFlow(item.id).subscribe({ next: () => { this.load(); this.msg.add({ severity: 'success', summary: 'Deleted' }); }, error: () => {} });
    } });
  }
