...
export class DocumentStatusesComponent implements OnInit {
  ...
  constructor(private svc: ProjectsStatusesService, private cd: ChangeDetectorRef, private msg: MessageService, private translate: TranslateService, private confirm: ConfirmationService) {}
  ...
  save(): void {
    ...
    if (this.isCreating) {
      this.svc.createStatus(payload, false).subscribe({ next: () => { this.displayDialog = false; this.load(); this.msg.add({ severity: 'success', summary: 'Created' }); this.loading = false; }, error: (e: any) => { this.error = e?.message || 'Error'; this.loading = false; } });
    } else {
      const id = this.editModel.id || this.editModel._id;
      this.svc.updateStatus(id, payload, false).subscribe({ next: () => { this.displayDialog = false; this.load(); this.msg.add({ severity: 'success', summary: 'Updated' }); this.loading = false; }, error: (e: any) => { this.error = e?.message || 'Error'; this.loading = false; } });
    }
  }

  confirmDelete(item: any): void {
    this.confirm.confirm({ message: `Delete ${item.name || item.id}?`, accept: () => {
      this.svc.deleteStatus(item.id, false).subscribe({ next: () => { this.load(); this.msg.add({ severity: 'success', summary: 'Deleted' }); }, error: () => {} });
    } });
  }
