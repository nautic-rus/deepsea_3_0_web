...
export class LoginComponent {
  ...
  onLogin(): void {
    ...
    if (!this.email || !this.password) {
      this.errorMessage = this.translate.instant('components.login.ERROR_ENTER_CREDENTIALS'); // TODO: make reactive (refresh on translate.onLangChange)
      return;
    }
  ...
  private getErrorMessage(err: HttpErrorResponse): string | null {
    ...
    const translate = (s: string) => {
      ...
      const map: Record<string, string> = {
        'invalid credentials': this.translate.instant('components.login.ERROR_INVALID_CREDENTIALS'), // TODO: make reactive (refresh on translate.onLangChange)
        'unauthorized': this.translate.instant('components.login.ERROR_UNAUTHORIZED'), // TODO: make reactive (refresh on translate.onLangChange)
        'forbidden': this.translate.instant('components.login.ERROR_FORBIDDEN') // TODO: make reactive (refresh on translate.onLangChange)
      };
      return map[key] ?? s;
    };
